...
#include "cleanInplace.cu"

//the algorithms
#include "inplaceRadixSelect.cu"
//#include "countingRadixSelect.cu"
//#include "countingWithExtraMemory.cu"
//#include "alternatingCounting.cu"
//#include "cleanCounting.cu"
#include "newRadix.cu"
...
//char* namesOfTimingFunctions[NUMBEROFALGORITHMS] = {"Inplace", "Destructive Radix Select", "NEW INPLACE"};
char* namesOfTimingFunctions[NUMBEROFALGORITHMS] = {"Inplace", "Destructive Radix Select"};

//char* namesOfTimingFunctions[NUMBEROFALGORITHMS] = {"New Inplace"};

using namespace std;
 template<typename T>
 struct isLarger_x {
  isLarger_x(T x) : x(x) {}
  __device__ bool operator()(T y) { return y > x; }

   T x;
};

template<typename T>
uint testIfKth(T *h_vec, uint size, uint k, T value){
      uint numLargerThanKth, numEqual;
  isLarger_x<T> isLargerFunctor(value);

    T *d_v;
    cudaMalloc(&d_v, sizeof(T) * size);
    cudaMemcpy(d_v, h_vec, size * sizeof(T), cudaMemcpyHostToDevice);
    thrust::device_ptr<T>d_ptr(d_v);

    numLargerThanKth = thrust::count_if(d_ptr, d_ptr + size, isLargerFunctor);
    numEqual = thrust::count(d_ptr, d_ptr + size,value);
    cudaFree(d_v);
    if(numLargerThanKth < k && numLargerThanKth + numEqual >= k){
      return 0;
    }
    else{
      return 1;
...
void compareAlgorithms(uint size, uint k, uint numTests,uint *algorithmsToTest, uint generateType,char* fileNamecsv){
  ...
  //  ptrToTimingFunction arrayOfTimingFunctions[NUMBEROFALGORITHMS] = {&timeInplaceRadixSelect<T>,&timeDestructiveRadixSelect<T>,&timeNewInplaceRadixSelect<T>};

  ptrToTimingFunction arrayOfTimingFunctions[NUMBEROFALGORITHMS] = {&timeInplaceRadixSelect<T>,&timeDestructiveRadixSelect<T>};
  ...
  for(i = 0; i < numTests; i++){
    ...
    for(x = 0; x < NUMBEROFALGORITHMS; x++){
      ...
      if(algorithmsToTest[j]){
        ...
        printf("TESTING: %u\n", j);
        temp = arrayOfTimingFunctions[j](h_vec,size,k);
    ...
    // std::cout << "KTH: " << tempResult << "\n";

    flag = testIfKth(h_vec, size, k, tempResult);
...
template<typename T>
void runTests(uint generateType, char* fileName,uint startPower, uint stopPower, uint timesToTestEachK, uint size){
  uint algorithmsToRun[NUMBEROFALGORITHMS]= {1,1};
  //  uint size=1180000000;
   //  uint size;
  uint i;
  uint arrayOfKs[25];
...
int main(int argc, char** argv)
{
  ...
  uint type,distributionType,startPower,stopPower, size=0;
  ...
  switch(type){
  case 1:
    //     runTests<float>(distributionType,fileName,startPower,stopPower,testCount,size);
    break;
  case 2:
    // runTests<double>(distributionType,fileName,startPower,stopPower,testCount);
    break;
  case 3:
    runTests<uint>(distributionType,fileName,startPower,stopPower,testCount,size);
    break;
  default:
