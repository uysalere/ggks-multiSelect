...
namespace NoExtremaRandomizedBucketSelect{
  using namespace std;
...
#define CUDA_CALL(x) do { if((x) != cudaSuccess) {      \
  ...
  T phaseOneR(T* d_vector, int length, int K, int blocks, int threads, int pass = 0){
    ...
    */
    if(K == 1){
      thrust::device_ptr<T>dev_ptr(d_vector);
      T minEdge = *(thrust::min_element(dev_ptr, dev_ptr + length));   
      return minEdge;
    }
    if(K == length){
      thrust::device_ptr<T>dev_ptr(d_vector);
      T maxEdge = *(thrust::max_element(dev_ptr, dev_ptr + length));   
      return maxEdge;
    }		
    /// ***********************************************************
    ...
    printf("noextrema kthbucketcount= %d\n", newInputLength);
  ...
  /**************************************************************************/
  template <typename T>
  T noExtremaRandomizedBucketSelectWrapper(T* d_vector, int length, int K, int blocks, int threads)
  {
    T kthValue;
