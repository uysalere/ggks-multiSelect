...
#define NUMBEROFMULTISELECTALGORITHMS 2

struct lineStruct{
  uint numAlgorithms;
  uint size;
  uint k;
  char arrayDistribution[100];
  char kDistribution[100];
  char* names[10];
  double ratios[10];
  uint errorFlag;
  float times[20];
...
void printLineStruct(lineStruct *line){
  ...
  printf("Size: %u\n", line->size);
  printf("Array Distribution: %s\n", line->arrayDistribution);
  printf("K Distribution: %s\n", line->kDistribution);
  printf("Error flag: %u\n", line->errorFlag);
  uint i;
  for(i = 0; i < line->numAlgorithms; i++){
     printf("Name: %s\n", line->names[i]);
...
lineStruct* readLine(FILE *fp, uint numberOfAlgorithms){
  ...
  //get the array distribution
  ptr =strtok(NULL, ",");
  sscanf(ptr,"%[100]s", &line->arrayDistribution);

  //get the k distribution
  ptr =strtok(NULL, ",");
  sscanf(ptr,"%[100]s", &line->kDistribution);
  ...
  for(i = 0; i < numberOfAlgorithms; i++){
	  line->ratios[i] = line->times[i]/ line->times[0];
  }
  ptr =strtok(NULL, ",");
...
lineStruct** gatherTests(char* baseFileName,uint &numTests){
  ...
  if (dpdf != NULL){
    while (epdf = readdir(dpdf)){
      if(! (strncmp(epdf->d_name, baseFileName, strlen(baseFileName)))){
        tempFn = (char*) malloc(200 * sizeof(char));
  ...
  uint linesRead = 0;
  uint i,j;
  uint numberOfAlgorithms = NUMBEROFMULTISELECTALGORITHMS;
  uint fieldCount = 0;
  uint numLines = 1;
  lineStruct *temp;
...
void printSizeKCombination(uint size, uint k, lineStruct **tests, uint numTests, uint numberOfAlgorithms,char *outputFile){
  ...
  for(i = 0; i < numberOfAlgorithms; i++){
    ...
    outFileStream << totalTimes[i] / testsOfThisType << "," << maxTimes[i] << "," << minTimes[i] << "," << timesWon[i] <<",";
  }
...
void printTestsSummary(uint numTests, lineStruct **tests, char *baseFileName){
  ...
  summaryFileName = (char*) malloc(300 * sizeof(char));
  ratioSummaryFileName = (char*) malloc(300 * sizeof(char));
  ratioOverviewFileName = (char*) malloc(300 * sizeof(char));

  overviewFileName = (char*) malloc(300 * sizeof(char));
